...
function getColorStyle({ variant, color, theme }: { variant: string; color: string; theme: Theme }) {
  ...
  switch (variant) {
    case 'contained':
      return { '&:hover': { backgroundColor: dark }, ...commonShadow };
    case 'shadow':
      return {
        color: contrastText,
        backgroundColor: main,
        boxShadow: shadows,
        '&:hover': { boxShadow: 'none', backgroundColor: dark },
        ...commonShadow
      };
    case 'outlined':
      return {
    ...
    default:
      return {
        color: dark,
        '&:hover': { color: darker ?? dark, backgroundColor: lighter ?? 'transparent' },
        ...commonShadow
      };
  }
}
      ...
      styleOverrides: {
        root: {
          ...
          '&::after': {
            ...
            borderRadius: 4,
            opacity: 0,
            // spec: ripple 100ms — RF-00-UX-DEFAULT-MOTION
            transition: 'all 0.1s'
          },
          '&:active::after': {
            position: 'absolute',
        ...
        contained: { '&.Mui-disabled': { ...disabledStyle } },
        outlined: {
          '&.Mui-disabled': {
            ...disabledStyle,
            '&:hover': {
              backgroundColor: p.grey[200],
              color: `${p.grey[300]} !important`,
              borderColor: 'inherit'
            }
          }
        },
        ...
        dashed: {
          ...
          '&.Mui-disabled': {
            color: `${p.grey[300]} !important`,
            borderColor: `${p.grey[400]} !important`,
            backgroundColor: `${p.grey[200]} !important`
          }
        },
        shadow: {
          ...primaryShadow,
        ...
        sizeExtraSmall: { minWidth: 56, fontSize: '0.625rem', padding: '2px 8px' },
        loading: {
          pointerEvents: 'none !important',
          '& svg': { width: 'inherit', height: 'inherit' },
          '&.MuiButton-loadingPositionCenter': { color: 'transparent !important' }
        }
      }
    }
  };
